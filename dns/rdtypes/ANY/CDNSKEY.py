...
def flags_to_text_set(flags):
    """Convert a CDNSKEY flags value to set texts
    @rtype: set([string])"""
...
def flags_from_text_set(texts_set):
    """Convert set of CDNSKEY flag mnemonic texts to CDNSKEY flag value
    @rtype: int"""
    ...
    for text in texts_set:
        try:
            ...
            raise NotImplementedError(
                "CDNSKEY flag '%s' is not supported" % text)
    return flags


class CDNSKEY(dns.rdata.Rdata):
    """CDNSKEY record
    ...
    """CDNSKEY is defined in RFC 7344"""

    __slots__ = ['flags', 'protocol', 'algorithm', 'key']

    def __init__(self, rdclass, rdtype, flags, protocol, algorithm, key):
        super(CDNSKEY, self).__init__(rdclass, rdtype)
        self.flags = flags
